  ...
  return (
    <div style={{ background: "var(--app-bg)", minHeight: "100vh", color: "var(--app-text)" }}>
      ...
      <div className="drug-page" style={{ maxWidth: 1200, margin: "0 auto", padding: "28px 32px 64px" }}>
        ...
        {/* ═══ ANSWER ROW — 2 panels ═══ */}
        <div className="drug-answer-row" style={{ display: "grid", gridTemplateColumns: "3fr 2fr", gap: 12, marginBottom: 20 }}>
          ...
          {(() => {
            ...
            const flag = FLAGS[userCountry] ?? "🌐";
            const cName = userCountryName[userCountry] ?? userCountry;
            ...
            return (
              <div style={{
                ...
                {/* ── Bottom: Predicted outlook ── */}
                {(() => {
                  // Active shortage, date known
                  if (myShortage && myStatus !== "anticipated" && myShortage.estimated_resolution_date) {
                    ...
                    const rangeStart = new Date(resDate);
                    rangeStart.setMonth(rangeStart.getMonth() - 2);
                    const rangeEnd = new Date(resDate);
                    rangeEnd.setMonth(rangeEnd.getMonth() + 2);
                    const rangeStartStr = rangeStart.toLocaleDateString("en-AU", { month: "short", year: "numeric" });
                    const rangeEndStr = rangeEnd.toLocaleDateString("en-AU", { month: "short", year: "numeric" });

                    return (
                      <div>
                        <div style={{ fontSize: 10, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.06em", color: "var(--app-text-4)", marginBottom: 6 }}>
                          Predicted return
                        </div>
                        <div style={{ display: "flex", alignItems: "baseline", gap: 10, marginBottom: 2 }}>
                          ...
                          <span style={{ fontSize: 12, color: "var(--teal)", fontFamily: "var(--font-dm-mono), monospace" }}>
                  ...
                  if (myShortage && myStatus !== "anticipated") {
                    return (
                      <div>
                        ...
                        <div style={{ fontSize: 18, fontWeight: 600, color: "var(--app-text)", marginBottom: 2 }}>
                          Resolution date unknown
                        </div>
                        <div style={{ fontSize: 12, color: "var(--app-text-3)" }}>
                          {myShortage.start_date
                            ? `Shortage ongoing since ${new Date(myShortage.start_date).toLocaleDateString("en-AU", { day: "numeric", month: "short", year: "numeric" })}`
                            : "Shortage ongoing"}
                        </div>
                      </div>
                    );
                  ...
                  if (myAnticipated || (myShortage && myStatus === "anticipated")) {
                    ...
                    return (
                      <div>
                        <div style={{ fontSize: 10, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.06em", color: "#d97706", marginBottom: 6 }}>
                          Anticipated shortage
                        </div>
                        <div style={{ display: "flex", alignItems: "baseline", gap: 10, marginBottom: 2 }}>
                          <span style={{ fontSize: 18, fontWeight: 600, color: "var(--app-text)" }}>
                            {impactRange || "Date TBC"}
                          </span>
                          {anticConf > 0 && (
                            <>
                              <span style={{ fontSize: 12, color: "var(--app-text-4)" }}>·</span>
                              <span style={{ fontSize: 12, color: "var(--teal)", fontFamily: "var(--font-dm-mono), monospace" }}>
                                Confidence {anticConf}/100
                              </span>
                            </>
                          )}
                        </div>
                  ...
                  return (
                    <div>
                      ...
                      <div style={{ fontSize: 18, fontWeight: 600, color: "var(--app-text)" }}>
                        Stable — no disruption predicted
                      </div>
                    </div>
                  );
                })()}
          ...
          {/* 2. GLOBAL STATUS — white bg, color used only on indicators */}
          <div style={{
            background: "#fff",
            ...
            {/* Country + critical summary */}
            <div style={{ fontSize: 11, color: "var(--app-text-4)", marginTop: 12 }}>
            ...
            {/* ── Bottom section: Risk Score or Monitoring ── */}
            <div style={{ height: 1, background: "var(--app-border)", margin: "14px 0" }} />
            {activeShortages.length > 0 ? (
              <div>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                  <span style={{ fontSize: 10, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.06em", color: "var(--app-text-4)" }}>
                    Supply Risk Score
                  </span>
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    <span style={{
                    ...
                    <span style={{
                      fontFamily: "var(--font-dm-mono), monospace",
                      fontSize: 18, fontWeight: 700, color: riskColors.color,
                    }}>
                      {drugRisk.riskScore}
                    </span>
                  </div>
                ...
                <div style={{
                  height: 4, borderRadius: 2, background: "var(--app-bg-3, #e2e8f0)",
                  overflow: "hidden", marginBottom: 6,
                }}>
                  <div style={{
                    width: `${drugRisk.riskScore}%`, height: "100%", borderRadius: 2,
                    background: drugRisk.riskScore >= 70
                      ? "var(--crit, #dc2626)"
                      : drugRisk.riskScore >= 40
                        ? "var(--med, #ca8a04)"
                        : "var(--low, #16a34a)",
                  }} />
                </div>
                <div style={{ fontSize: 11, color: "var(--app-text-3)" }}>
                  {drugRisk.primarySignal}
                </div>
              </div>
            ) : (
              <div>
        ...
        <div className="drug-two-col" style={{ display: "grid", gridTemplateColumns: "3fr 2fr", gap: 24, alignItems: "start" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            {/* AI INSIGHT — interactive client component */}
            <AiInsightChips
              drugContext={{
                ...
                anticipatedCount: anticipatedShortages.length,
                affectedCountries: [...affectedCountries] as string[],
                anticipatedCountries: [...anticipatedCountries] as string[],
                worstSeverity: worstSeverity,
                alternatives: alternatives.map((a: { drugs?: { generic_name?: string }; similarity_score?: number }) => ({
                  name: a.drugs?.generic_name ?? "Unknown",
