...
| Major scraper files exist but never wired to cron (israel_moh, malaysia_npra, poland_mz, portugal_infarmed, turkey_titck, uae_mohap, greece_eof, belgium_famhp, hk_drugoffice, argentina_anmat) | High | Each is a meaningful coverage gain — pharma market size, geopolitical relevance. |
| `china_nmpa` (tier-3 upstream signal) wired to daily cron 2026-06-03 but **low yield**: the English NMPA portal it scrapes is policy-heavy and rarely publishes API-suspension/GMP-revocation notices. The timely, complete enforcement feed (飞行检查 flight inspections, GMP cert revocations) is on the WAF-protected Chinese portal `www.nmpa.gov.cn` — needs a headless browser (Playwright) to scrape. | Medium | See module docstring in `backend/scrapers/china_nmpa_scraper.py`. Headless Chinese-site scraping is the high-value upgrade. |
